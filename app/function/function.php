...
function format_curency($val){
 ...
 function upload_image_post($file, $path){
        ...
        if($ext == "jpg" || $ext == "jpeg" || $ext == "png" || $ext == "gif"){
            ...
            $nameimg = changeTitle($filename).'.'.$file->getClientOriginalExtension(); 
            $hinh = "".str_random(6)."_".$nameimg;
            while(file_exists("public/img/original/".$hinh))
            {
                $hinh = "".str_random(6)."_".$nameimg;
            } 
			...
			// $image_resize->insert(public_path('images/wartermark.png'));
		    $image_resize->save(public_path('img/'.$path.'/100x100/'.$hinh));
            // 200x200 
			...
			// $image_resize->insert(public_path('images/wartermark.png'));
		    $image_resize->save(public_path('img/'.$path.'/300x300/'.$hinh));
            // 200x200 
		    ...
		    $image_original->save(public_path('img/'.$path.'/800x800/'.$hinh));
		    // original
